...
function InstrumentSelector({ isOpen, onClose, onSelectInstrument, currentInstrument, isLoadingInstrument }) {
  ...
  // Drag-to-scroll handlers for the horizontal categories bar
  ...
  const onCategoryPointerMove = (e) => {
    ...
    if (!isDragging && Math.abs(deltaX) > threshold) {
      ...
      // Start capturing only once we actually begin dragging
      try {
        categoriesRef.current?.setPointerCapture?.(e.pointerId)
      } catch (_) {
    ...
    if (isDragging || Math.abs(deltaX) > threshold) {
      ...
      // Prevent text selection and default scrolling behavior while actively dragging
      e.preventDefault()
    }
  }

  const endCategoryDrag = (e) => {
    if (!isPressedRef.current && !isDragging) return
    isPressedRef.current = false
    setIsDragging(false)
    try {
      categoriesRef.current?.releasePointerCapture?.(e.pointerId)
    } catch (_) {
      // noop if not supported
    }
  }
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-zinc-700 bg-zinc-800/50">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-white">Choose Instrument</h2>
            <button
              onClick={onClose}
        ...
        <div className="px-6 py-3 border-b border-zinc-800 bg-zinc-900/50">
          <div
            ref={categoriesRef}
            className={`flex gap-2 overflow-x-auto scrollbar-hide select-none ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {isLoading ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-zinc-500 text-lg">Loading instruments...</div>
            </div>
        ...
        <div className="px-6 py-4 border-t border-zinc-800 bg-zinc-900/50">
          <div className="text-xs text-zinc-500 text-center">
            {isLoadingInstrument ? 'Loading instrument...' : 'Click an instrument to select'}
          </div>
        </div>
      </div>
    </div>
